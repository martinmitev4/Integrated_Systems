...
namespace MovieApp.Controllers
{
    public class TicketsController : Controller
    {
        ...
        public async Task<IActionResult> AddToCart(Guid? id)
        {
            ...
            var shoppingCart = _context.ShoppingCarts.FirstOrDefault(x => x.OwnerId == userId);

            TicketInShoppingCart ps = new TicketInShoppingCart();

            if (ticket != null && shoppingCart != null)
            {
                ps.TicketId = ticket.Id;
                ps.ShoppingCartId = shoppingCart.Id;
            }
        ...
        [HttpPost]
        public async Task<IActionResult> AddConfirmed(TicketInShoppingCart model)
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);

            var shoppingCart = _context.ShoppingCarts.FirstOrDefault(x => x.OwnerId == userId);

            if (shoppingCart.TicketInShoppingCart == null)
                shoppingCart.TicketInShoppingCart = new List<TicketInShoppingCart>(); ;

            shoppingCart.TicketInShoppingCart.Add(model);
            await _context.SaveChangesAsync();
